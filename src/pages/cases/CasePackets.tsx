...
export default function CasePackets() {
  ...
  const buildHtml = () => {
    ...
    const parts: string[] = [];
    if (selected.includes("overview")) parts.push(`<section><h2>Case overview</h2><p><strong>${esc(activeCase?.title)}</strong></p><p>${esc(`${activeCase?.matter_type ?? ""} · ${activeCase?.jurisdiction ?? ""}`)}</p><p>${esc(activeCase?.jurisdiction)}</p></section>`);
    ...
    if (selected.includes("people")) parts.push(`<section><h2>People &amp; organizations</h2>${rows([...snapshot.people, ...snapshot.organizations], (p) => `<div class="item"><h3>${esc(p.display_name || p.name)}</h3><p class="meta">${esc(p.role_label || p.org_type || p.organization || "")}</p></div>`)}</section>`);
